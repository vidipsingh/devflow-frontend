...
const TABS = [
  ...
  { id: "pulls", label: "Pull Requests", icon: (
    <svg className="w-3.5 h-3.5 cursor-pointer" viewBox="0 0 16 16" fill="currentColor">
...
export default function RepositoryDetailPage({
  ...
  // ── Pull Requests inline state ────────────────────────────────────────────
  const { prs, total: prsTotal, loading: prsLoading, error: prsError, fetchPRs } = usePullRequests(name);
  const [prTab, setPrTab] = useState<"open" | "closed" | "merged">("open");
  ...
  // Always fetch open PRs on mount so the tab badge shows the count
  useEffect(() => {
    loadPRs("open");
  ...
  useEffect(() => {
    if (activeTab === "pulls") loadPRs(prTab);
  }, [prTab, activeTab, loadPRs]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-5">
            ...
            <div className="flex space-x-1">
               <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-3 rounded-lg bg-[#1a1a2e] hover:bg-[#1e1e35] border border-white/[0.08] text-white/70 hover:text-white text-sm font-medium transition-colors cursor-pointer"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                  </svg>
                  Clone
                  <svg className={`w-3 h-3 transition-transform ${cloneOpen ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {cloneOpen && (
                  <>
                    {/* backdrop */}
                    <div className="fixed inset-0 z-10" onClick={() => setCloneOpen(false)} />
                    <div className="absolute right-0 top-full mt-2 z-20 w-96 rounded-xl border border-white/[0.08] bg-[#0d0d14] shadow-2xl p-4 space-y-3">
                      <p className="text-white/60 text-xs font-medium uppercase tracking-wider">Clone with DevFlow CLI</p>
                      {/* CLI command box */}
                      <div className="flex items-center gap-2 bg-[#0a0a0f] border border-white/[0.06] rounded-lg px-3 py-2">
                        <svg className="w-3.5 h-3.5 text-white/30 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 9l3 3-3 3m5 0h3" />
                        </svg>
                        <code className="flex-1 text-xs text-indigo-300 font-mono truncate select-all">
                          {`devflow clone ${typeof window !== "undefined" ? window.location.href : ""}`}
                        </code>
                        <button
                          onClick={() => {
                            const url = typeof window !== "undefined" ? window.location.href : "";
                            navigator.clipboard.writeText(`devflow clone ${url}`);
                            setCloneCopied(true);
                      ...
                      <p className="text-white/25 text-xs">
                        Make sure you are logged in: <code className="text-indigo-300/70 font-mono">devflow auth login</code>
                      </p>
                    </div>
                  </>
                )}
        ...
        {activeTab === "pulls" && (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <h2 className="text-sm font-medium text-white/60">
                {prsTotal > 0 ? `${prsTotal} ${prTab} pull request${prsTotal !== 1 ? "s" : ""}` : "Pull Requests"}
              </h2>
              <Link
                href={`/dashboard/repositories/${name}/pulls/new`}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-500 hover:bg-indigo-400 text-white text-xs font-medium transition-colors cursor-pointer"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                </svg>
            ...
            <div className="border border-white/[0.08] rounded-xl overflow-hidden bg-[#111117]">
              {/* Open / Closed / Merged tabs */}
              <div className="flex items-center gap-1 px-4 py-2 border-b border-white/[0.06] bg-white/[0.02]">
                <button
                  onClick={() => setPrTab("open")}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors cursor-pointer ${
                    prTab === "open" ? "text-white bg-white/[0.07]" : "text-white/40 hover:text-white/70"
                  }`}
                >
                  <svg className="w-3 h-3 text-emerald-400" viewBox="0 0 16 16" fill="currentColor">
                    <path d="M1.5 3.25a2.25 2.25 0 1 1 3 2.122v5.256a2.251 2.251 0 1 1-1.5 0V5.372A2.25 2.25 0 0 1 1.5 3.25Zm5.677-.177L9.573.677A.25.25 0 0 1 10 .854V2.5h1A2.5 2.5 0 0 1 13.5 5v5.628a2.251 2.251 0 1 1-1.5 0V5a1 1 0 0 0-1-1h-1v1.646a.25.25 0 0 1-.427.177L7.177 3.427a.25.25 0 0 1 0-.354ZM3.75 2.5a.75.75 0 1 0 0 1.5.75.75 0 0 0 0-1.5Zm0 9.5a.75.75 0 1 0 0 1.5.75.75 0 0 0 0-1.5Zm8.25.75a.75.75 0 1 0 1.5 0 .75.75 0 0 0-1.5 0Z" />
                  </svg>
                  {prTab === "open" ? `${prsTotal} Open` : "Open"}
                </button>
                <button
                  onClick={() => setPrTab("closed")}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors cursor-pointer ${
                    prTab === "closed" ? "text-white bg-white/[0.07]" : "text-white/40 hover:text-white/70"
                  }`}
                >
                  <svg className="w-3 h-3 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                  </svg>
                  {prTab === "closed" ? `${prsTotal} Closed` : "Closed"}
                </button>
                <button
                  onClick={() => setPrTab("merged")}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors cursor-pointer ${
                    prTab === "merged" ? "text-white bg-white/[0.07]" : "text-white/40 hover:text-white/70"
                  }`}
                >
                  <svg className="w-3 h-3 text-violet-400" viewBox="0 0 16 16" fill="currentColor">
                  ...
                  {prTab === "merged" ? `${prsTotal} Merged` : "Merged"}
                </button>
              </div>
              ...
              {prsLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-white/[0.04] border border-white/[0.07] flex items-center justify-center">
                    <svg className="w-6 h-6 text-white/20" viewBox="0 0 16 16" fill="currentColor">
                      <path d="M1.5 3.25a2.25 2.25 0 1 1 3 2.122v5.256a2.251 2.251 0 1 1-1.5 0V5.372A2.25 2.25 0 0 1 1.5 3.25Zm5.677-.177L9.573.677A.25.25 0 0 1 10 .854V2.5h1A2.5 2.5 0 0 1 13.5 5v5.628a2.251 2.251 0 1 1-1.5 0V5a1 1 0 0 0-1-1h-1v1.646a.25.25 0 0 1-.427.177L7.177 3.427a.25.25 0 0 1 0-.354ZM3.75 2.5a.75.75 0 1 0 0 1.5.75.75 0 0 0 0-1.5Zm0 9.5a.75.75 0 1 0 0 1.5.75.75 0 0 0 0-1.5Zm8.25.75a.75.75 0 1 0 1.5 0 .75.75 0 0 0-1.5 0Z" />
                    </svg>
                  </div>
                  <div>
                    <p className="text-white/50 text-sm font-medium">No {prTab} pull requests</p>
                    <p className="text-white/25 text-xs mt-1">
                      {prTab === "open" ? "Propose changes by opening a pull request." : prTab === "merged" ? "No pull requests merged yet." : "No closed pull requests."}
                    </p>
                  </div>
                  {prTab === "open" && (
                    <Link
                ...
                <>
                  {prs.map((pr: PullRequest) => {
                    const stateColor = pr.state === "merged" ? "text-violet-400" : pr.state === "closed" ? "text-red-400" : "text-emerald-400";
                    return (
                      <div
                        key={pr.id}
                        className="flex items-start gap-3 px-4 py-3 hover:bg-white/[0.02] transition-colors border-b border-white/[0.05] last:border-0"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start gap-2 flex-wrap">
                            <Link
                              href={`/dashboard/repositories/${name}/pulls/${pr.number}`}
                              className="text-sm font-medium text-white/85 hover:text-indigo-300 transition-colors leading-snug"
                            >
                              {pr.title}
                            </Link>
                            {pr.isDraft && (
                              <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs text-white/35 bg-white/[0.05] border border-white/[0.08]">Draft</span>
                            )}
                            {(pr.labels ?? []).map((l: PRLabel) => {
                              const bg = l.color ? (l.color.startsWith("#") ? l.color : "#" + l.color) : "#6366f1";
                              return (
                                <span
                                  key={l.name}
                          ...
                          <p className="text-white/30 text-xs mt-0.5">
                            #{pr.number} · {pr.state === "merged" ? "merged" : pr.state === "closed" ? "closed" : "opened"} by{" "}
                            <span className="text-white/45">{pr.authorName}</span>
                            <span className="ml-1.5 text-white/20 font-mono text-[10px]">
